...
ACPICA_C_FLAGS= $(COMMON_C_FLAGS) $(FLAGS_64) -include include/infinity_acenv.hpp -include include/infinity_acenvex.hpp

COMMON_LINK_FLAGS=-lgcc

KERNEL_LINK_FLAGS=$(COMMON_LINK_FLAGS) -T linker.ld

TLIB_FLAGS=$(COMMON_CPP_FLAGS) $(FLAGS_64) $(WARNING_FLAGS) -mcmodel=small -fPIC -ffunction-sections -fdata-sections -DINFINTY_TLIB=yes
TLIB_LINK_FLAGS=$(COMMON_CPP_FLAGS) $(FLAGS_64) $(WARNING_FLAGS) -mcmodel=small -fPIC -Wl,-gc-sections

PROGRAM_FLAGS=$(COMMON_CPP_FLAGS) $(FLAGS_64) $(WARNING_FLAGS) -I../../tlib/include/ -I../../printf/include/  -static -L../../tlib/debug/ -ltlib -mcmodel=small -fPIC -DINFINTY_PROGRAM=yes
...
debug/$(1)/%.cpp.d: $(1)/%.cpp
	@ mkdir -p debug/$(1)/
	@ $(CXX) $(KERNEL_CPP_FLAGS_64) $(INFINTY_FLAGS) $(WARNING_FLAGS) -MM -MT debug/$(1)/$$*.cpp.o $$< | sed -e 's@^\(.*\)\.o:@\1.d \1.o:@' > $$@

debug/$(1)/%.cpp.o: $(1)/%.cpp
	@ mkdir -p debug/$(1)/
	@ echo -e "$(MODE_COLOR)[debug]$(NO_COLOR) Compile $(FILE_COLOR)$(1)/$$*.cpp$(NO_COLOR)"
	@ $(CXX) $(KERNEL_CPP_FLAGS_64) $(INFINTY_FLAGS) $(WARNING_FLAGS) -c $$< -o $$@
...
debug/acpica/source/components/$(1)/%.c.d: acpica/source/components/$(1)/%.c
	...
	@ $(CXX) $(ACPICA_C_FLAGS) $(INFINTY_FLAGS) -MM -MT acpica/source/components/$(1)/$$*.c.o $$< | sed -e 's@^\(.*\)\.o:@\1.d \1.o:@' > $$@
...
debug/acpica/source/components/$(1)/%.c.o: acpica/source/components/$(1)/%.c
	...
	@ $(CC) $(ACPICA_C_FLAGS) $(INFINTY_FLAGS) -c $$< -o $$@
