...
uint64_t* create_buddy_array(size_t managed_space, size_t block){
    ...
    infinity_assert(virtual_address, "Impossible to allocate virtual pages for the physical allocator");

    infinity_assert(paging::map_pages(virtual_address, physical_address, pages), "Impossible to map pages for the physical allocator");
...
size_t physical_allocator::allocate(size_t blocks){
    infinity_assert(blocks * paging::PAGE_SIZE < free(), "Not enough physical memory");
