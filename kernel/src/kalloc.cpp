...
public:
    ...
    malloc_footer_chunk* footer() const {
        infinity_assert(aligned(reinterpret_cast<uintptr_t>(static_cast<const volatile void*>(this))), "kalloc: Invalid alignment");
...
void expand_heap(malloc_header_chunk* current, uint64_t bytes = 0){
    ...
    infinity_assert(block, "Impossible to allocate block for expand_heap");
...
void kalloc::k_free(void* block){
    ...
    infinity_assert(!free_header->is_free(), "kalloc: free block getting freed\n");
