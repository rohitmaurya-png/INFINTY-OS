...
size_t early_map_page(size_t physical){
    infinity_assert(paging::virtual_early_page < 0x100000, "Invalid early page");
...
bool paging::map(size_t virt, size_t physical, uint8_t flags){
    ...
    auto pml4t = find_pml4t();
    infinity_assert(reinterpret_cast<uintptr_t>(pml4t[pml4e]) & PRESENT, "A PML4T entry is not PRESENT");

    auto pdpt = find_pdpt(pml4t, pml4e);
    infinity_assert(reinterpret_cast<uintptr_t>(pdpt[pdpte]) & PRESENT, "A PDPT entry is not PRESENT");

    auto pd = find_pd(pdpt, pdpte);
    infinity_assert(reinterpret_cast<uintptr_t>(pd[pde]) & PRESENT, "A PD entry is not PRESENT");
