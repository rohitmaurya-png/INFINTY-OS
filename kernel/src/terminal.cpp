...
void stdio::virtual_terminal::send_input(char key) {
    ...
    infinity_assert(!keyboard_buffer.full(), "keyboard buffer is full!");
...
void stdio::virtual_terminal::send_mouse_input(std::keycode key) {
    ...
    infinity_assert(!mouse_buffer.full(), "mouse buffer is full!");
...
size_t stdio::virtual_terminal::read_input_raw(size_t ms) {
    ...
    infinity_assert(!raw_buffer.empty(), "There is a problem with the sleep queue: Nothing to read from raw_buffer");
