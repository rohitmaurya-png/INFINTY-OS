...
path::path(std::string_view path) : base(path.begin(), path.end()) {
    infinity_assert(path.size(), "Invalid base path");

    update_positions(base, positions);
}

path::path(const path& base_path, std::string_view p) : base(base_path.base) {
    infinity_assert(p.empty() || p[0] != '/', "Impossible to add absolute path to another path");
...
path::path(const path& base_path, const path& p) : base(base_path.base) {
    infinity_assert(p.is_relative(), "Impossible to add absolute path to another path");
...
std::string_view path::sub_root_name() const {
    infinity_assert(is_absolute(), "sub_root_name() does not make sense on relative path");
