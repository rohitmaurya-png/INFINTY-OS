...
network::ip::address network::arp::cache::get_ip(uint64_t mac) const {
    infinity_assert(is_mac_cached(mac), "The MAC is not cached in the ARP table");
    ...
    infinity_unreachable("The MAC is not cached in the ARP table");
}

uint64_t network::arp::cache::get_mac(network::ip::address ip) const {
    infinity_assert(is_ip_cached(ip), "The IP is not cached in the ARP table");
    ...
    infinity_unreachable("The IP is not cached in the ARP table");
}
