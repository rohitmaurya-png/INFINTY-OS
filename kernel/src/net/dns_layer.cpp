...
void prepare_packet_query(network::packet& packet, uint16_t identification) {
    ...
    dns_header->auinfinityity_rrs  = switch_endian_16(0);
...
void network::dns::layer::decode(network::interface_descriptor& /*interface*/, network::packet_p& packet) {
    ...
    auto auinfinityity_rrs  = switch_endian_16(dns_header->auinfinityity_rrs);
    ...
    logging::logf(logging::log_level::TRACE, "dns: Auinfinityithy RRs %u \n", size_t(auinfinityity_rrs));
