...
#ifdef INFINTY_CONFIG_SCHED_VERBOSE
...
void queue_process(scheduler::pid_t pid){
    infinity_assert(pid < scheduler::MAX_PROCESS, "pid out of bounds");

    auto& process = pcb[pid];

    infinity_assert(process.process.priority <= scheduler::MAX_PRIORITY, "Invalid priority");
    infinity_assert(process.process.priority >= scheduler::MIN_PRIORITY, "Invalid priority");
...
size_t select_next_process_with_lock(){
    ...
    infinity_assert(current_priority > 0, "The idle task should always be ready");
    ...
    infinity_unreachable("No process is READY");
}
...
void scheduler::kill_current_process(){
    ...
    infinity_unreachable("A killed process has been run!");
}
...
void scheduler::yield(){
    infinity_assert(started, "No interest in yielding before start");
    infinity_assert(pcb[current_pid].state == process_state::RUNNING, "Can only yield() running processes");
...
void scheduler::reschedule(){
    infinity_assert(started, "No interest in rescheduling before start");
...
void scheduler::block_process(pid_t pid){
    infinity_assert(is_started(), "The scheduler is not started");
    infinity_assert(pid < scheduler::MAX_PROCESS, "pid out of bounds");
    infinity_assert(pid != idle_pid, "No reason to block the idle task");
...
void scheduler::unblock_process(pid_t pid){
    ...
    infinity_assert(pid < scheduler::MAX_PROCESS, "pid out of bounds");
    infinity_assert(pid != idle_pid, "No reason to unblock the idle task");
    infinity_assert(is_started(), "The scheduler is not started");
    infinity_assert(pcb[pid].state == process_state::BLOCKED || pcb[pid].state == process_state::BLOCKED_TIMEOUT || pcb[pid].state == process_state::WAITING, "Can only unblock BLOCKED/WAITING processes");
...
void scheduler::sleep_ms(pid_t pid, size_t time){
    infinity_assert(pid < scheduler::MAX_PROCESS, "pid out of bounds");
    infinity_assert(pcb[pid].state == process_state::RUNNING, "Only RUNNING processes can sleep");
...
scheduler::process_t& scheduler::create_kernel_task(const char* name, char* user_stack, char* kernel_stack, void (*fun)()){
    ...
    infinity_assert((reinterpret_cast<size_t>(process.context) - sizeof(interrupt::syscall_regs)) % STACK_ALIGNMENT == 0, "Process context must be correctly aligned");
    infinity_assert((process.context->rsp - sizeof(interrupt::syscall_regs)) % STACK_ALIGNMENT == 0, "Process user stack must be correctly aligned");
    infinity_assert(process.kernel_rsp % STACK_ALIGNMENT == 0, "Process kernel stack must be correctly aligned");
