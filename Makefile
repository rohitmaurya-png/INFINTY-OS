...
infinity.flp: hdd.img bootloader/stage1.bin bootloader/stage2.bin init/debug/init.bin kernel/debug/kernel.bin programs
	mkdir -p mnt/fake/
...
qemu: default
	touch virtual.log
	sudo qemu-system-x86_64 -enable-kvm -cpu host -serial file:virtual.log -netdev tap,helper=/usr/libexec/qemu-bridge-helper,id=infinity_net0 -device rtl8139,netdev=infinity_net0,id=infinity_nic0 -vga std -hda hdd.img &
...
qemu_user: default
	touch virtual.log
	sudo qemu-system-x86_64 -enable-kvm -cpu host -serial file:virtual.log -net user,vlan=0 -net nic,model=rtl8139,vlan=0,macaddr=52:54:00:12:34:56 -net socket,listen=localhost:1234 -net dump,vlan=0,file=infinity.pcap -vga std -hda hdd.img &
	echo "Reading kernel log (Ctrl+C for exit)"
	tail -f virtual.log
	kill %1
